...
author:
- \emph{Mikael Randrup Byrialsen}^[Aalborg University, randrup@business.aau.dk, MaMTEP, Department of Business and Management.] 
...
- Find data for BNP for Danmark for perioden 1966-2020
  - Gå ind på statistikbanken <https://www.statbank.dk/statbank5a/default.asp?w=1440>
  - Klik på 'Nationalregnskab og offentlige finanser' under emner
  - Klik på 'Årligt nationalregnskab'
  - Klik på 'Nøgletal, årligt nationalregnskab'
  - Vælg datasæt 'NAN1'
  - Vælg tidsserien: 'B.1*g BNP' $\rightarrow$ prisenhed: '2010-priser' $\rightarrow$ År: 'markér alle'
  - Klik på 'VIS TABEL'
  - Klik på 'gem som' '*.xlsx' og gem filen på computeren

## Håndtering af data i excel
- Gør data klar til import i R
  - Åben datasættet i excel
  - Markér området B3:BE4 og kopiér cellerne
...
- Indlæs data i R
  ...
  - Klik på 'Filer' $\rightarrow$ 'Import Dataset' $\rightarrow$ 'From EXCEL' $\rightarrow$ 'Browse'(find filen manuelt)
...
View(BNP) 
```
  - Undersøg datasæt (kan ligeledes findes i toppen)
  - Klik på pilen ved siden af navnet på datasættet for at undersøge egenskaberne ved data (55 obs., 2 variable (Aar og BNP))
...
- Plot en figur med 'Aar' på 1. aksen og 'BNP' på 2. aksen
...
- Eksportér figur som PDF/PNG
  - Plots $\rightarrow$ Export $\rightarrow$ save as image/pdf
...
```
- Forklaring:
    - x angiver variabel 1
    - z angiver variabel 3
