...
- brug "namess_from" og "values_from" til at få dit dataset. 
...
**MEN** så vil jeg ikke kunne bruge ggplot2, da dette ikke er lavet for tidsserier. Derfor beholder vi formatet "option_2_data" hvor jeg har en time variabel med. og vi kan nu lave forskellige plots:
...
```{r}
ggplot(option_2_data, aes(x= time)) +
geom_line(aes(y = DNK, col= "DNK")) +
geom_line(aes(y = DEU, col= "DEU"))

```


```{r}
plot1 <- ggplot(option_2_data, aes(time)) +
geom_line(aes(y = DNK))

plot2 <- ggplot(option_2_data, aes(time)) +
geom_line(aes(y = DEU))
```
